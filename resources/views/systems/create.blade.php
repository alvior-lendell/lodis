...
<head>
    ...
    <title>Add System - LODISv2</title>
...
<body class="min-h-screen font-sans antialiased bg-slate-50 text-slate-900 flex flex-col">
    ...
    <header class="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <span class="text-xs font-semibold text-slate-300">/</span>
                <span class="text-xs font-bold text-slate-700 uppercase tracking-wider">Register Platform</span>
            </div>
    ...
    <main class="flex-1 max-w-3xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
            <h2 class="text-xl font-extrabold text-slate-900 tracking-tight">Register New Application</h2>
            <p class="text-xs text-slate-500 mt-1 mb-6">Add an operational system launcher entry to the LODISv2 main dashboard portal.</p>

            <form method="POST" action="{{ route('systems.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required
                        ...
                        placeholder="e.g. LORISv2 Portal">
                ...
                <div>
                    ...
                    <div class="flex rounded-xl border border-slate-200 overflow-hidden focus-within:border-brand focus-within:ring-1 focus-within:ring-brand shadow-sm transition">
                        ...
                        <input type="text" name="url" id="url" value="{{ preg_replace('/^https?:\/\//i', '', old('url')) }}" required
                            class="w-full py-2.5 px-3 text-xs font-medium text-slate-900 outline-none font-mono"
                            placeholder="loris.lendell-local.xyz">
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-brand focus:ring-1 focus:ring-brand text-xs font-medium text-slate-900 shadow-sm outline-none transition"
                        placeholder="Brief overview of system functions or target department operations.">{{ old('description') }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="default_sort_order" id="default_sort_order" value="{{ old('default_sort_order', $nextSortOrder) }}" min="0" required
                        ...
                        <span class="text-[11px] text-slate-400 mt-1 block">Auto-incremented to next position. Lower numbers display first.</span>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <div id="logo-preview-box" class="w-24 h-20 rounded-xl border border-dashed border-slate-300 bg-slate-50 flex items-center justify-center shrink-0 overflow-hidden">
                            <span class="text-[10px] text-slate-400 font-semibold text-center px-1" id="logo-preview-text">Preview</span>
                            <img id="logo-preview-img" src="#" alt="Logo Preview" class="hidden max-h-16 w-auto object-contain">
                        </div>
                        ...
                        <div class="flex-1">
                            ...
                            <span class="text-[11px] text-slate-400 mt-1 block">PNG, SVG, JPG, WEBP up to 2MB.</span>
                        </div>
                    </div>
