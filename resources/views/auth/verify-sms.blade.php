@php
    ...
    $verifyRoute = $verifyRoute ?? (Route::has('verification.sms.verify') ? route('verification.sms.verify') : route('password.sms.verify'));
    $resendRoute = $resendRoute ?? (Route::has('verification.sms.resend') ? route('verification.sms.resend') : route('password.sms.send'));
    $title = $title ?? 'SMS Security Verification';
    $buttonLabel = $buttonLabel ?? 'Verify Phone Number';
@endphp
