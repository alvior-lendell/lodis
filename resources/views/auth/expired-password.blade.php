...
<head>
    ...
    <title>Password Expired - LODISv2</title>
        ...
        <div class="order-1 lg:order-2 lg:w-1/2 flex items-center justify-center p-6 sm:p-10 lg:p-16 bg-white relative">
            <div id="auth-container" class="w-full max-w-md transition-all duration-200">
                ...
                <div class="mb-8 text-center lg:text-left">
                    <h3 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Password Expired</h3>
                    <p class="text-xs sm:text-sm text-slate-500 mt-1">
                        Your account password has exceeded the maximum allowed age of {{ $securitySettings->password_expiry_days }} days. You must update it to continue.
                    </p>
                </div>

                <!-- Warning Session Alert -->
                @if (session('warning'))
                    <div class="mb-6 p-4 rounded-2xl bg-amber-50 border border-amber-200/80 text-xs text-amber-800 flex items-start gap-3 shadow-sm">
                ...
                <form id="expired-password-form" method="POST" action="{{ route('password.expired.update') }}" autocomplete="off" class="space-y-5 transition-opacity duration-200">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="relative rounded-2xl shadow-sm">
                            ...
                            <button type="button" id="toggle-current-password-btn" title="Toggle current password visibility"
                                class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-brand focus:text-brand outline-none transition">
                                <svg id="current-eye-icon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                                <svg id="current-eye-slash-icon" class="w-5 h-5 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        <div class="relative rounded-2xl shadow-sm">
                            ...
                            <button type="button" id="toggle-password-btn" title="Toggle new password visibility"
                                class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-brand focus:text-brand outline-none transition">
                    ...
                    <div>
                        ...
                        <div class="relative rounded-2xl shadow-sm">
                            ...
                            <button type="button" id="toggle-confirm-password-btn" title="Toggle password confirmation visibility"
                                class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-brand focus:text-brand outline-none transition">
                    ...
                    <!-- Dynamic Password Policy Checklist with Data Attributes -->
                    <div id="password-checklist-container"
                        ...
                        <span class="block text-[11px] font-bold text-slate-600 uppercase tracking-wider">Password Requirements</span>
                        ...
                        @if (($securitySettings->password_history_limit ?? 0) > 0)
                            <div class="flex items-center gap-2 text-slate-500 pt-2 border-t border-slate-200/60 font-medium">
                                <svg class="w-4 h-4 shrink-0 text-slate-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/></svg>
                                <span>Cannot reuse current or last {{ $securitySettings->password_history_limit }} passwords</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-2">
                        <button id="submit-btn" type="submit" disabled
                            ...
                            <span id="btn-label">Update Password & Proceed</span>
                        </button>
                    </div>
                </form>
                ...
                <div class="mt-6 text-center">
                    <form method="POST" action="{{ route('logout') }}" autocomplete="off">
                        @csrf
                        <button type="submit" class="text-xs font-bold text-slate-500 hover:text-slate-700 transition">
                            Sign out of session
                        </button>
                    </form>
                </div>
            ...
            <div class="relative z-10">
                ...
                <div class="max-w-md my-auto">
                    ...
                    <p class="text-slate-600 text-xs sm:text-sm leading-relaxed mb-8">
                        To maintain corporate compliance and secure single-sign-on access, passwords must be rotated every {{ $securitySettings->password_expiry_days }} days.
                    </p>
                </div>
            </div>
