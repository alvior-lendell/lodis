        ...
        <div class="flex items-center gap-3">
            ...
            <div class="relative" id="notification-menu-wrapper">
                <button type="button" id="notification-menu-button" 
                    ...
                    <span id="notif-badge-counter" 
                        class="{{ $unreadCount > 0 ? '' : 'hidden' }} absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-[9px] font-bold text-white ring-2 ring-white">
                ...
                <div id="notification-menu-dropdown" class="hidden absolute right-0 top-12 w-80 sm:w-96 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50">
                    <div class="px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span id="notif-header-badge" class="{{ $unreadCount > 0 ? '' : 'hidden' }} px-2 py-0.5 rounded-full text-[10px] font-bold bg-brand-light text-brand">
                                <span id="notif-header-count">{{ $unreadCount }}</span> New
                            </span>
                        </div>
                        @if (Route::has('notifications.markAllRead'))
                            <form method="POST" action="{{ route('notifications.markAllRead') }}" id="mark-all-read-form" class="{{ $unreadCount > 0 ? '' : 'hidden' }}">
                                @csrf
                    ...
                    <div id="notification-items-list" class="max-h-80 overflow-y-auto divide-y divide-slate-100">
                        @forelse ($notifications as $notification)
                            <a href="{{ Route::has('notifications.read') ? route('notifications.read', $notification->id) : ($notification->data['url'] ?? '#') }}" 
                               class="p-3.5 text-xs text-slate-700 hover:bg-slate-50 flex items-start gap-3 transition">
                        ...
                        @empty
                            <div id="notification-empty-state" class="p-6 text-center text-slate-500">
            ...
            <div class="relative" id="user-menu-wrapper">
                <button type="button" id="user-menu-button" 
                    ...
                    @if ($userPhoto)
                        <img src="{{ $userPhoto }}" alt="{{ $fullName }}" class="w-7 h-7 rounded-lg object-cover shrink-0 border border-slate-200">
                    @else
                ...
                <!-- Dropdown Card -->
                <div id="user-menu-dropdown" class="hidden absolute right-0 top-12 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50">
                    <div class="px-4 py-2.5 border-b border-slate-100 flex items-center gap-3">
                        @if ($userPhoto)
                            <img src="{{ $userPhoto }}" alt="{{ $fullName }}" class="w-9 h-9 rounded-xl object-cover shrink-0 border border-slate-200">
                        @else
                            <div class="w-9 h-9 rounded-xl bg-brand-light text-brand font-bold text-sm flex items-center justify-center shrink-0 uppercase">
                                {{ strtoupper(substr($firstName ?: $fullName, 0, 1)) }}
                            </div>
                        @endif
                        <div class="min-w-0 flex-1">
                            <p class="text-xs font-bold text-slate-900 truncate">{{ $fullName }}</p>
                            <span class="text-[10px] font-semibold text-slate-500 uppercase block mt-0.5">{{ $userRole }}</span>
                        </div>
                    </div>
...
<script>
    if (typeof window.headerDropdownInitialized === 'undefined') {
        ...
        const getBaseTitle = () => document.title.replace(/^(\(\d+\+?\)\s*|🔔\s*\(?\d+\+?\)?\s*)/, '').trim();
        ...
        window.startTitleFlashing = function (flashText = 'New Notification!') {
            ...
            titleFlashTimer = setInterval(() => {
                document.title = toggle ? `${badgeStr} ${baseTitle}`.trim() : `🔔 ${flashText}`;
                toggle = !toggle;
            }, 1000);
        };
        ...
        window.stopTitleFlashing = function () {
            ...
            if (currentUnreadCount > 0) {
                ...
                document.title = `${badgeStr} ${baseTitle}`;
            } else {
                document.title = baseTitle;
            }
        };

        window.addEventListener('focus', () => window.stopTitleFlashing());
        document.addEventListener('click', () => window.stopTitleFlashing());

        // Increment notification badge & prepend item dynamically
        window.incrementNotificationBadge = function(title, message, url) {
            currentUnreadCount += 1;

            const bellBadge = document.getElementById('notif-badge-counter');
            if (bellBadge) {
                bellBadge.textContent = currentUnreadCount > 9 ? '9+' : currentUnreadCount;
            ...
            if (headerBadge && headerCount) {
                headerCount.textContent = currentUnreadCount;
            ...
            if (itemsList) {
                const itemHtml = `
                    <a href="${url || '#'}" class="p-3.5 text-xs text-slate-700 hover:bg-slate-50 flex items-start gap-3 transition bg-amber-50/50">
                        <div class="w-8 h-8 rounded-lg bg-brand-light text-brand shrink-0 flex items-center justify-center font-bold text-xs mt-0.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[11px] text-slate-500 line-clamp-2 mt-0.5">${message}</p>
                            <span class="text-[10px] text-slate-400 block mt-1 font-mono">Just now</span>
                        </div>
                    </a>
                `;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Reverb Listener (routes to the GET /auth/device-review/{id} review page)
                ...
                if (typeof window.Echo !== 'undefined') {
                    window.Echo.private('user-security.{{ auth()->id() }}')
                        .listen('.device.approval.requested', (e) => {
                            ...
                            if (data) {
                                const title = 'New Workstation Sign-In Request';
                                const msg = `Authorization requested for ${data.device_name || 'Workstation'} (${data.ip_address || 'N/A'})`;
                                
                                // Route through readAndRedirect endpoint using notification database ID
                                const readUrl = `/notifications/${data.notification_id || data.id}/read`;
                    
                                window.incrementNotificationBadge(title, msg, readUrl);
                                window.playNotificationChime(title, msg);
                                window.startTitleFlashing('New Security Alert!');
                            }
                        });
                } else {
                    setTimeout(registerEchoListener, 300);
                }
            };
